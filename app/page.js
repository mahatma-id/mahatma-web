...
export default function Home() {
  ...
  return (
    <div className="text-slate-800 bg-white overflow-x-hidden selection:bg-orange-500 selection:text-white">
      ...
      {/* 2. OUR MISSION */}
      <section className="py-20 bg-slate-50 text-center px-6 md:px-12 lg:px-16 border-b border-slate-100">
        <div className="container mx-auto max-w-4xl">
            <span className="text-orange-600 font-bold tracking-widest uppercase text-xs mb-4 block">Our Mission</span>
            ...
            <p className="text-lg md:text-xl text-slate-500 leading-relaxed mb-10 font-light max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-8 md:gap-16 font-bold text-lg text-slate-800 uppercase tracking-widest">
                <div className="flex flex-col items-center hover:scale-110 transition-transform"><span className="text-orange-500 text-4xl mb-3">🌍</span> People</div>
                <div className="flex flex-col items-center hover:scale-110 transition-transform"><span className="text-orange-500 text-4xl mb-3">🌱</span> Planet</div>
                <div className="flex flex-col items-center hover:scale-110 transition-transform"><span className="text-orange-500 text-4xl mb-3">🚀</span> Progress</div>
            </div>
        </div>
      </section>

      {/* 3. OUR SERVICE */}
      <section id="layanan" className="py-24 bg-white px-6 md:px-12 lg:px-16">
        <div className="container mx-auto max-w-7xl">
            <div className="flex flex-col lg:flex-row justify-between items-center gap-12 mb-16">
                <div className="lg:w-1/2 text-center lg:text-left">
                    <span className="text-orange-600 font-bold tracking-widest uppercase text-xs mb-4 block">Our Service</span>
                ...
                {settings.serviceImageUrl && (
                    <div className="lg:w-1/2 w-full">
                        <img src={settings.serviceImageUrl} alt="Our Service" className="w-full h-64 md:h-80 object-cover rounded-[2rem] shadow-xl" />
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {services.map(svc => (
                    <div key={svc.id} className="bg-slate-50 p-8 md:p-10 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-xl hover:-translate-y-2 transition-all duration-500 group flex flex-col h-full cursor-pointer">
                        <div className="w-14 h-14 bg-white shadow-sm text-orange-600 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-orange-600 group-hover:text-white group-hover:rotate-12 transition-all duration-500">
          ...
          <section id="tim" className="py-20 bg-slate-900 text-white px-6 md:px-12 lg:px-16">
            <div className="container mx-auto max-w-7xl">
                <div className="text-center mb-12">
                    <span className="text-orange-500 font-bold tracking-widest uppercase text-xs mb-4 block">Meet The Experts</span>
                    <h2 className="text-3xl md:text-4xl font-bold mb-4">Orang-Orang Hebat di Balik Mahatma</h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {teams.map(member => (
                        <div key={member.id} className="group relative overflow-hidden rounded-3xl bg-slate-800">
                            <img src={member.img} alt={member.name} className="w-full h-80 object-cover group-hover:scale-110 group-hover:opacity-60 transition-all duration-700" />
                            ...
                            <div className="absolute bottom-0 left-0 p-6 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                                ...
                                <p className="text-orange-400 text-[10px] font-bold tracking-widest uppercase">{member.role}</p>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      <section id="insight" className="py-20 bg-white px-6 md:px-12 lg:px-16 border-b border-slate-100">
        <div className="container mx-auto max-w-7xl">
            <div className="mb-12 text-center md:text-left">
                <span className="text-orange-600 font-bold tracking-widest uppercase text-xs mb-4 block">Our Insight</span>
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mb-4 leading-tight">Wawasan & Perspektif Terbaru</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {posts.map(post => {
                    let dStr = "";
                    ...
                    <Link href={`/berita/${post.id}`} key={post.id} className="group flex flex-col cursor-pointer bg-slate-50 rounded-3xl p-4 hover:bg-white hover:shadow-xl hover:-translate-y-2 transition-all duration-500 border border-transparent hover:border-slate-100">
                        <div className="w-full h-60 bg-slate-200 rounded-2xl overflow-hidden mb-5 relative">
                            ...
                            <span className="absolute top-4 left-4 bg-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest rounded-full shadow-md text-slate-900">{post.category}</span>
                        </div>
                        <div className="px-3 pb-3 flex-1 flex flex-col">
                            <h3 className="text-xl md:text-2xl font-bold text-slate-900 mb-3 group-hover:text-orange-600 transition leading-snug">{post.title}</h3>
                            <p className="text-[10px] text-slate-500 font-bold tracking-widest uppercase mt-auto pt-4 border-t border-slate-200">{dStr} <span className="mx-2 text-orange-300">•</span> {post.author || 'Admin'}</p>
                        </div>
                    </Link>
                )})}
            </div>
      ...
      {testimonials.length > 0 && (
          <section className="py-20 bg-slate-50 px-6 md:px-12 lg:px-16 overflow-hidden">
            <div className="container mx-auto max-w-7xl">
                <div className="text-center mb-12">
                    <span className="text-orange-600 font-bold tracking-widest uppercase text-xs mb-4 block">Testimonials</span>
                    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Apa Kata Mereka Tentang Kami</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {testimonials.map(testi => (
                        <div key={testi.id} className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-100 relative group hover:-translate-y-2 transition-all duration-300">
                            <span className="absolute top-4 right-6 text-5xl text-slate-100 font-serif group-hover:text-orange-100 transition-colors">"</span>
                            <p className="text-slate-600 text-sm md:text-base italic leading-relaxed mb-8 relative z-10">{testi.text}</p>
                            <div className="flex items-center gap-4 border-t border-slate-100 pt-6">
                                <div className="w-12 h-12 rounded-full bg-slate-900 text-white flex justify-center items-center font-bold text-base">{testi.name[0]}</div>
                                <div><h4 className="font-bold text-slate-900 text-sm md:text-base">{testi.name}</h4><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{testi.company}</p></div>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      {faqs.length > 0 && (
          <section className="py-20 bg-white px-6 md:px-12 lg:px-16 border-t border-slate-100">
            <div className="container mx-auto max-w-3xl">
                <div className="text-center mb-12">
                    <span className="text-orange-600 font-bold tracking-widest uppercase text-xs mb-4 block">F.A.Q</span>
                    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Pertanyaan Paling Sering Diajukan</h2>
                </div>
                <div className="space-y-4">
                    {faqs.map((faq, idx) => (
                        <div key={faq.id} className="border border-slate-200 rounded-2xl overflow-hidden transition-all duration-300">
                            <button onClick={() => toggleFaq(idx)} className={`w-full text-left p-6 md:p-8 font-bold text-base md:text-lg flex justify-between items-center transition-colors ${openFaq === idx ? 'bg-orange-50 text-orange-600' : 'bg-white text-slate-900 hover:bg-slate-50'}`}>
                                {faq.q} <span className={`transform transition-transform text-xl ${openFaq === idx ? 'rotate-45' : ''}`}>+</span>
                            </button>
                            <div className={`px-6 md:px-8 overflow-hidden transition-all duration-500 ease-in-out ${openFaq === idx ? 'max-h-96 py-6 border-t border-orange-100' : 'max-h-0 py-0'}`}><p className="text-slate-600 text-base leading-relaxed font-light">{faq.a}</p></div>
                        </div>
                    ))}
                </div>
            </div>
      ...
      {/* CALL TO ACTION */}
      <section id="kontak" className="bg-slate-900 text-white pt-24 pb-20 px-6 md:px-12 lg:px-16 border-t-[8px] border-orange-600 relative overflow-hidden">
        ...
        <div className="container mx-auto max-w-3xl text-center relative z-10">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-[1.2]">Siap Untuk <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">Berubah?</span></h2>
            <p className="text-base md:text-xl text-slate-300 max-w-2xl mx-auto mb-10 font-light leading-relaxed">Bergabunglah dalam perjalanan pertumbuhan, keberlanjutan, dan perubahan positif. Wujudkan masa depan di mana organisasi Anda berkembang dengan cepat.</p>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                <button className="w-full sm:w-auto px-10 py-5 bg-orange-600 text-white font-bold tracking-widest uppercase rounded-full text-xs hover:bg-orange-500 hover:-translate-y-1 transition duration-300 shadow-[0_10px_40px_rgba(234,88,12,0.4)]">Pesan Layanan</button>
                <a href={`https://wa.me/${waNumber}`} target="_blank" className="w-full sm:w-auto px-10 py-5 bg-white/10 text-white font-bold tracking-widest uppercase rounded-full text-xs hover:bg-white hover:text-slate-900 transition duration-300 backdrop-blur-sm border border-white/20">Hubungi WhatsApp</a>
            </div>
        </div>
      </section>

      {/* FOOTER DINAMIS */}
      <footer className="bg-white pt-20 pb-10 px-6 md:px-12 lg:px-16 border-t border-slate-200">
        <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 mb-16">
                ...
                <div className="lg:col-span-4 lg:pr-8">
                    ...
                    <p className="text-slate-500 text-sm md:text-base leading-loose mb-8">
                    ...
                    <div className="flex items-center gap-2 text-slate-500 mb-2">
                        ...
                        <span className="text-sm font-bold">Speak to our expert at</span>
                    </div>
                    <a href={`https://wa.me/${waNumber}`} target="_blank" className="text-2xl font-bold text-orange-600 hover:text-orange-700 transition">
                        {rawPhone}
                    </a>
                </div>

                <div className="lg:col-span-2">
                    <h4 className="font-bold text-slate-900 mb-6 uppercase tracking-wider text-sm">Follow Us</h4>
                ...
                <div className="lg:col-span-2">
                    <h4 className="font-bold text-slate-900 mb-6 uppercase tracking-wider text-sm">Mitra Kerja</h4>
                                ...
                                {partners.slice(0, 15).map(p => (
                                    <div key={p.id} className="w-full aspect-square flex items-center justify-center bg-white rounded border border-slate-100 hover:border-orange-200 p-1 transition-colors">
                                        {p.imgUrl ? (
                                            ...
                                            <span title={p.name} className="text-[9px] font-bold text-slate-400 uppercase w-full text-center truncate">{p.name.substring(0,3)}</span>
                                        )}
                                    </div>
                                ))}
                            </div>
