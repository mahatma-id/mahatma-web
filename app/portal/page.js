...
export default function PortalClient() {
  ...
  // State Login Session & Data Dokumen
  ...
  useEffect(() => {
    // Ambil logo untuk header
    const unsubSettings = onSnapshot(doc(db, "settings", "general"), snap => {
        if(snap.exists()) setSettings(snap.data());
    });
    
    // Cek apakah sudah login (sederhana pakai localStorage)
  ...
  // Mengambil daftar syarat dokumen dan dokumen klien saat sudah login
  useEffect(() => {
      if (loggedInClient) {
  ...
  // Fungsi Upload ke Cloudinary (Menggunakan 'auto' agar bisa terima PDF & Gambar)
  ...
  const handleFileUpload = async (e, masterId) => {
      ...
      // Validasi ukuran maksimal (contoh: 5MB)
      ...
      try {
          ...
          // Cek apakah klien ini sudah pernah upload dokumen ini (Jika ya, kita update. Jika belum, kita insert baru)
          const existingDoc = clientDocs.find(d => d.masterId === masterId);
          
          if (existingDoc) {
              await updateDoc(doc(db, "client_docs", existingDoc.id), {
                  fileUrl: fileUrl,
                  status: 'pending', // Status kembali pending agar admin cek lagi
  ...
  const handleRegister = async (e) => {
    ...
    try {
        ...
        alert("Pendaftaran Berhasil! Menunggu persetujuan Admin MASE sebelum bisa login.");
        setIsLoginMode(true);
        setEmail(''); setPassword(''); setLembagaName(''); setPicName(''); setPhone('');
    } catch (err) {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            if (clientData.status === 'pending') {
                alert("Akun Anda sedang diverifikasi oleh Admin. Harap tunggu.");
            } else if (clientData.status === 'rejected') {
                alert("Mohon maaf, pendaftaran akun Anda ditolak.");
            } else {
                  ...
                  <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                          {docMasters.length === 0 ? (
                              ...
                              // Cari apakah dokumen ini sudah diupload
                                  ...
                                  <div key={master.id} className={`p-5 rounded-2xl border ${uploadedDoc?.status === 'approved' ? 'border-emerald-200 bg-emerald-50/30' : uploadedDoc?.status === 'revision' ? 'border-red-200 bg-red-50/30' : 'border-slate-200 bg-white'} shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all`}>
                                      ...
                                      <div className="flex-1">
                                          ...
                                          <div className="ml-9 flex flex-col items-start gap-2">
                                              ...
                                              {/* Jika ada komentar revisi dari Admin */}
                                              ...
                                              {/* Link untuk melihat file yang sudah diupload */}
                                              {uploadedDoc && (
                                      ...
                                      <div className="md:w-64 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-4">
                                          {/* Form Upload Muncul jika belum ACC */}
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl relative z-10 overflow-hidden">
        ...
        <div className="bg-slate-50 p-8 text-center border-b border-slate-100 relative">
            ...
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Portal Klien ISO</h1>
            <p className="text-xs text-slate-500 mt-2">Sistem Verifikasi & Audit Dokumen</p>
        </div>

        {/* Tab Toggle */}
        <div className="flex border-b border-slate-100">
            <button onClick={() => setIsLoginMode(true)} className={`flex-1 py-4 text-xs font-bold uppercase tracking-widest transition-colors ${isLoginMode ? 'bg-white text-emerald-600 border-b-2 border-emerald-600' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>Login</button>
            <button onClick={() => setIsLoginMode(false)} className={`flex-1 py-4 text-xs font-bold uppercase tracking-widest transition-colors ${!isLoginMode ? 'bg-white text-emerald-600 border-b-2 border-emerald-600' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>Daftar Baru</button>
        </div>
        ...
        <div className="p-8">
            {isLoginMode ? (
                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Email Lembaga / PIC</label>
                        <input type="email" required value={email} onChange={e=>setEmail(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="email@lembaga.com" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Password</label>
                        <input type="password" required value={password} onChange={e=>setPassword(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="••••••••" />
                    </div>
                    <button disabled={loading} className="w-full bg-slate-900 text-white font-bold tracking-widest uppercase text-xs py-4 rounded-xl hover:bg-emerald-600 transition shadow-lg mt-4">
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                    <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Nama Lembaga / Perusahaan</label>
                        <input type="text" required value={lembagaName} onChange={e=>setLembagaName(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="Cth: PT Sukses Bersama" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Nama PIC (Penanggung Jawab)</label>
                        <input type="text" required value={picName} onChange={e=>setPicName(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="Nama Lengkap" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">No. WhatsApp</label>
                            <input type="text" required value={phone} onChange={e=>setPhone(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="0812..." />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Email</label>
                            <input type="email" required value={email} onChange={e=>setEmail(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="email@..." />
                        </div>
                    </div>
                    <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1">Password</label>
                        <input type="password" required value={password} onChange={e=>setPassword(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-emerald-500 outline-none text-sm transition-colors" placeholder="Buat password untuk login" />
                    </div>
                    <button disabled={loading} className="w-full bg-emerald-600 text-white font-bold tracking-widest uppercase text-xs py-4 rounded-xl hover:bg-emerald-700 transition shadow-lg mt-4">
                        {loading ? 'Memproses...' : 'Daftarkan Lembaga'}
                    </button>
                    <p className="text-[9px] text-slate-400 text-center leading-relaxed">Setelah mendaftar, akun Anda harus disetujui (ACC) oleh tim Admin sebelum bisa digunakan untuk login.</p>
                </form>
            )}
        </div>
      </div>
