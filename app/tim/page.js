...
export default function TimPage() {
  ...
  useEffect(() => {
    ...
    // Ambil Data Pengaturan (Logo, dll)
    ...
    return () => { unsubSettings(); unsubTeams(); unsubPartners(); };
  }, []);
  ...
  return (
    <div className="text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-950 min-h-screen flex flex-col transition-colors duration-300">
      ...
      {/* --- HEADER SOLID (Sama persis dengan Halaman Berita) --- */}
      <header className="bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 sticky top-0 z-[100] transition-all duration-300 py-3">
        <div className="container mx-auto px-4 md:px-12 lg:px-16 flex justify-between items-center max-w-7xl">
          ...
          <nav className="hidden lg:flex items-center gap-10 font-bold text-xs tracking-widest uppercase text-slate-600 dark:text-slate-300">
            ...
            <Link href="/tim" className="text-emerald-500 hover:-translate-y-1 transition-all">Our Team</Link>
        ...
        <div className={`lg:hidden absolute top-full left-0 w-full bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 shadow-xl transition-all duration-300 ease-in-out overflow-hidden ${isMobileMenuOpen ? 'max-h-96 py-4 opacity-100' : 'max-h-0 py-0 opacity-0 pointer-events-none'}`}>
            <nav className="flex flex-col items-center gap-4 font-bold text-sm tracking-widest uppercase text-slate-600 dark:text-slate-300 px-4">
                ...
                <Link href="/tim" onClick={() => setIsMobileMenuOpen(false)} className="text-emerald-600 w-full text-center pb-2 border-b border-slate-50 dark:border-slate-800">Our Team</Link>
      ...
      <main className="flex-grow pt-12 pb-20 px-4 md:px-12 lg:px-16">
        <div className="container mx-auto max-w-7xl">
            ...
            <div className="text-center mb-12 md:mb-16">
                ...
                <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-slate-900 dark:text-white mb-4">Our Experts</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
                {teams.map(member => (
                    <div key={member.id} className="group relative overflow-hidden rounded-2xl md:rounded-3xl bg-slate-800 dark:bg-slate-900 aspect-[3/4] shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer border border-transparent dark:border-slate-800">
                        ...
                        <div className="absolute bottom-0 left-0 p-3 md:p-6 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 w-full text-center">
                            <h3 className="text-sm md:text-lg font-bold mb-1 text-white leading-tight">{member.name}</h3>
            ...
            {teams.length === 0 && (
                <p className="text-center text-slate-500 dark:text-slate-400 py-20">Belum ada data tim yang ditambahkan.</p>
            )}

        </div>
      </main>

      {/* FOOTER LENGKAP (Sama Persis dengan Home) */}
